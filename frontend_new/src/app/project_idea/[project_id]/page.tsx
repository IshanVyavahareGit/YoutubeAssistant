...
function ProjectIdeaSubmission({params} : {params: {project_id: string};}) {
  ...
  useEffect(() => {
    ...
    const fetchTitles = async () => {
      if(project_id){
        try {
            const response = await fetch(`http://localhost:8080/apis/get_idea/${project_id}`, {
              method: 'POST',  
              headers: {
            ...
            if (data.success === "True" && data["idea_title"] && data["idea_desc"]) {
              setDateCreated(data["date_created"]);
              setIdeaTitle(data["idea_title"]);
              setIdeaDescription(data["idea_desc"]);
            }
          } catch (error) {
            console.error("Error fetching titles:", error);
            setMessage("Failed to load idea.");
            setMessageColor("text-red-500");
          }
      }
      else{
        router.push(`/project_idea`);
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response1 = await fetch("http://localhost:8080/apis/update_project", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (data.success === "True") {
        ...
        const response = await fetch(`http://localhost:8080/apis/gen_titles/${project_id}`, {
          method: 'POST',  
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        else router.push(`/titles_page/${project_id}`);
        ...
        let errorMessage = "An error occurred while trying to update the project idea.";
